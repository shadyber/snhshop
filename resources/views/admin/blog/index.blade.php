...
@section('title','List Blog Posts')
@section('content')
    <div class="card shadow mb-10">
        <!-- Card Header - Dropdown -->
        <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
            <h6 class="m-0 font-weight-bold text-primary">List All Blog Post </h6>
                            ...
                            <table class="table table-bordered dataTable" id="dataTable" width="100%" cellspacing="0" role="grid" aria-describedby="dataTable_info" style="width: 100%;">
                                ...
                                @foreach($blogs as $blog)
                                    <tr role="row" class="odd">
                                        <td>
                                            <img src="{{$blog->thumb}}" alt="{{$blog->title}}" class="img-responsive img-thumbnail" width="48px">
                                        </td>
                                        <td class="sorting_1">{{$blog->title}}</td>

                                        <td>
                                            <form method="POST" action="/blogs/{{$blog->id}}" class="form-inline ">
                                                {{ csrf_field() }}
                                                {{ method_field('DELETE') }}

                                                <div class="form-group">
                                                    <a href="/blogs/{{$blog->id}}/edit" class="btn btn-default"><i class="fa fa-pen"></i></a>
                                                    <a href="/blogs/{{$blog->slug}}" class="btn btn-primary"><i class="fa fa-eye"></i></a>
